...
class CanceledRidesVC: UIViewController
{
    ...
    var aryCanceledRidesHistory : NSArray?
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.reloadTableView), name: NSNotification.Name(rawValue: NotificationCenterName.keyForCanceledRides), object: nil)
    }
    ...
    @objc func reloadTableView() {
        DispatchQueue.main.async {
            self.aryCanceledRidesHistory = SingletonClass.sharedInstance.aryCancelRides
...
extension CanceledRidesVC : UITableViewDataSource, UITableViewDelegate
{
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == ((self.aryCanceledRidesHistory?.count ?? 0) - 5) {
            if !isDataLoading{
                isDataLoading = true
